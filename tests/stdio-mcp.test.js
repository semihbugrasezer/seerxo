...
describe("stdio MCP process", () => {
  ...
  it("negotiates, lists parity tools, calls tools, and keeps stdout JSON-only", async () => {
    ...
    assert.equal(
      generated.result.structuredContent.title,
      generationFixture.data.title,
    );
  ...
  it("masks internal errors and logs them to stderr", async () => {
    const child = spawn(process.execPath, ["bin/seerxo.js"], {
      cwd: new URL("..", import.meta.url),
      env: {
        ...process.env,
    ...
    assert.equal(errorResponse.error.message, "Internal JSON-RPC error");
    assert.ok(stderr.includes("Unknown tool: unknown_tool"));
  });
});
